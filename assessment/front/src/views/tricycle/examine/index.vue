  ...
  <div class="search">
    <Card v-show="currView=='index'">
      ...
      <div v-if="this.total!=0">
        <Row :gutter="32">
          <Col span="10">
            <img :src="imgSrc" style="height:300px;width:500px;" />
            <img :src="imgSrc1" style="height:300px;width:500px;" />
          </Col>
          <Col span="12">
            <Row :gutter="10">
              <Form label-colon :label-width="90">
                <FormItem label="店名">
                  <Input readonly v-model="nowSelectData.shopName" style="width: 260px" />
                </FormItem>
                <FormItem label="店铺负责人">
                  <Input readonly v-model="nowSelectData.shopDuty" style="width: 260px" />
                </FormItem>
                <FormItem label="负责人手机号">
                  <Input readonly v-model="nowSelectData.mobile" style="width: 260px" />
                </FormItem>
                <FormItem label="负责人身份证号">
                  <Input readonly v-model="nowSelectData.idCard" style="width: 260px" />
                </FormItem>
                <FormItem label="店铺地址">
                  <Input v-model="nowSelectData.address" style="width: 260px" />
                </FormItem>
                <FormItem label="营业执照号码">
                  <Input readonly v-model="nowSelectData.yyzz" style="width: 260px" />
                </FormItem>
                <FormItem label="备注">
                  <Input readonly v-model="nowSelectData.remark" style="width: 260px" />
                </FormItem>
                <FormItem label="注册时间">
                  <Input readonly v-model="nowSelectData.date" style="width: 260px" />
                </FormItem>
              </Form>
            </Row>
          </Col>
  ...
  methods: {
    ...
    backData(){
      if(this.nowSelectNumber == 0){
        ...
        this.imgSrc = 'https://changjienongye.cn/yyzz/' + this.nowSelectData.yyzzFile;
    ...
    nextData(){
      if(this.nowSelectNumber == this.total - 1){
        ...
        this.imgSrc1 = 'https://changjienongye.cn/dp/' + this.nowSelectData.addressFile;
        if(this.nowSelectData.hasImage == '1'){
          this.imgSrc1 = 'https://changjienongye.cn/dp/' + this.nowSelectData.addressFile + '.jpg';
        }
      }
    },
    passFx(){
    ...
    passFx2(){
      ...
      // console.log(this.nowSelectData.id);
      // console.log(this.nowSelectData.money);
      pass({id:this.nowSelectData.id,msg:"NO",msg:this.disPassReason}).then(res => {
        that.nextData();
      })
    },
    openDisPassFxWindow(){
    ...
    getDataList() {
      ...
      getSellerNotShList().then(res => {
        ...
        if (res.success) {
          ...
          // console.log(res);
          that.imgSrc = 'https://changjienongye.cn/yyzz/' + this.nowSelectData.yyzzFile;
          that.imgSrc1 = 'https://changjienongye.cn/dp/' + this.nowSelectData.addressFile;
