  ...
  <div class="search">
    ...
    <Card v-show="currView=='index'">
      <Row @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="0" class="search-form">
          <Form-item prop="shopName">
            <Input type="text" v-model="searchForm.shopName" placeholder="请输入店名" clearable style="width: 200px"/>
          </Form-item>
          <Form-item prop="shopDuty">
            <Input type="text" v-model="searchForm.shopDuty" placeholder="请输入店铺负责人" clearable style="width: 200px"/>
          </Form-item>
          <Form-item prop="mobile">
            <Input type="text" v-model="searchForm.mobile" placeholder="负责人手机号" clearable style="width: 200px"/>
          </Form-item>
          <Form-item prop="idCard">
            <Input type="text" v-model="searchForm.idCard" placeholder="负责人身份证号" clearable style="width: 200px"/>
          </Form-item>
          <Form-item prop="address">
            <Input type="text" v-model="searchForm.address" placeholder="请输入店铺地址" clearable style="width: 200px"/>
          </Form-item>
          <Form-item prop="yyzz">
            <Input type="text" v-model="searchForm.yyzz" placeholder="工商营业执照号码" clearable style="width: 200px"/>
          </Form-item>
          <Form-item class="br">
            <Button
              @click="handleSearch"
    ...
    <Card v-show="currView=='index'">
      <div v-if="this.nowSelectCarList.length==0">
        ...
        <p class="error404-body-con-message">{{itemMsg}}</p>
...
<script>
import { getSellerList, deleteSeller } from "./api.js";
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getSellerList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteSeller({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
